...
main = do
  ...
  print $ solve 10000 $ parse input

solve :: Int -> Array MonkeyIndex Monkey -> Integer
...
parse input = listArray (0, length monkeys - 1) betterHandledMonkeys
  where
    betterHandledMonkeys = map (\m@Monkey {op = defaultOp} -> m {op = manageStressOp defaultOp}) monkeys

    manageStressOp = \op worry -> op (worry `mod` divLcm)
    divLcm = foldl1 lcm (map divTest monkeys)
